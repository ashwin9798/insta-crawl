...
def get_user_info(browser):
  ...
  container = browser.find_element_by_class_name('_m558i')
  # img_container = browser.find_element_by_class_name('_b0acm')

  # infos = container.find_elements_by_class_name('_t98z6')

  alias_name = container.find_element_by_class_name('_5b1eb')\
  ...
  return alias_name
# , prof_img, num_of_posts, followers, following
...
def extract_post_info(browser):
  ...
  link = post.find_element_by_class_name('_eeohz')
  linkToUser = link.find_element_by_tag_name('a').text
  ...
  return linkToUser, img, tags, int(likes), int(len(comments) - 1)
...
def extract_information(browser, username):
  ...
  alias_name = get_user_info(browser)
  ...
  for link in links:
    ...
    try:
      linkToUser, img, tags, likes, comments = extract_post_info(browser)

      post_infos.append({
        'linkToUser': "instagram.com/"+linkToUser,
        'img': img,
        'tags': tags,
        'likes': likes,
        'comments': comments
